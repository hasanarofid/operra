    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center gap-4">
                <Link :href="route('admin.tickets.index')" class="text-gray-500 hover:text-gray-700">
                    &larr; Back
                </Link>
                <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                    {{ ticket.subject }} <span class="text-sm font-normal text-gray-500">#{{ ticket.id }}</span>
                </h2>
                <span class="text-sm ml-auto text-gray-500">User: {{ ticket.user?.name }} ({{ ticket.user?.email }})</span>
            </div>
        </template>
                    ...
                    <div ref="messagesContainer" class="flex-1 p-6 overflow-y-auto space-y-4 bg-gray-50 dark:bg-gray-900">
                        <div v-for="msg in ticket.messages" :key="msg.id" 
                            ...
                            <div class="p-3 rounded-lg shadow-sm"
                                 :class="msg.user_id === $page.props.auth.user.id ? 'bg-indigo-600 text-white' : 'bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200'">
                                <p class="whitespace-pre-wrap text-sm">{{ msg.message }}</p>
                            </div>
                            <span class="text-[10px] text-gray-400 mt-1">
                                {{ msg.user_id === $page.props.auth.user.id ? 'Admin (You)' : (msg.user?.name || 'User') }} • {{ new Date(msg.created_at).toLocaleString() }}
                            </span>
                        </div>
                    </div>
